...
for i in range(m):
    for j in range(n):
        ...
        array2[image[i][j][0]]+=1
        array3[image[i][j][0]]+=1
        count+=1
for i in range(256):
    array1[i]/=count
    array2[i]/=count
...
for i in range(m):
    for j in range(n):
        image[i][j][0]=array1[image[i][j][0]]
        image[i][j][1]=array2[image[i][j][1]]
        image[i][j][2]=array3[image[i][j][2]]
...
for i in range(m):
    for j in range(n):
        image_hsv[i][j][2]=array[image_hsv[i][j][2]]
